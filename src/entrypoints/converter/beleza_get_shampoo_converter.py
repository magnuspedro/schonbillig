...
class BelezaGetShampooConverter(BelezaAbstractConverter):
    def to_entity(self, response: Response) -> Shampoo:
        ...
        return Shampoo(
            ...
            size=size,
            texture=self.clear(shampoo_specs.get(InfoLine.TEXTURE.value)) or None,
            price=[
                Price(**{'price': float(price), 'source': source})],
            utility=self.clear(shampoo_specs.get(InfoLine.UTILITY.value)),
            size_unit=self.clear(shampoo_specs.get(InfoLine.SIZE.value)),
            hair_type=self.clear(shampoo_specs.get(
                InfoLine.HAIR_TYPE.value)),
            hair_shaft_condition=self.clear(shampoo_specs.get(
